...
def run(
    ...
    if DEBUG:
        print "create inst[]"
    inst = create_instruction_signals(  # inst[0]: 入力,  inst[1]: 出力
        n_in=n1,
        n_out=n3,
        depth=depth,
        p=p,
        rng=rng)
    if DEBUG:
        print "inst[0].shape:", inst[0].shape
    if DEBUG:
        print "inst[1].shape:", inst[1].shape
    ...
    if DEBUG:
        print "W0.shape:", network.W[0].shape
    if DEBUG:
        print "W1.shape:", network.W[1].shape

    s31 = np.dot(inst[1], inst[0].T)  # Sigma31
    if DEBUG:
        print "S31 (shape:", s31.shape, ")\n", s31
    s11 = np.dot(inst[0], inst[0].T)  # Sigma11
    if DEBUG:
        print "S31 (shape:", s11.shape, ")\n", s11
    ...
    if DEBUG:
        print "VVT", np.dot(V, V.T)
    if DEBUG:
        print "s31 ?=", np.dot(U, np.dot(S, V))
    ...
    # 学習出来てるか，試しに走らせてみる
    if DEBUG:
        print "test run network after learning"
    network.set_W((W0, W1))
    ...
    if DEBUG:
        print "final err(output, inst[1]):", err
